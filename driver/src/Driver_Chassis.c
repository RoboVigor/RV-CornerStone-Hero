...
void Chassis_Calculate_Real_Speed(ChassisData_Type *cd, float *motor_Speed) {
    float coefficient = (CHASSIS_INVERSE_WHEEL_RADIUS * CHASSIS_MOTOR_REDUCTION_RATE);
    cd->realvx        = -1 * (-motor_Speed[0] - motor_Speed[1] + motor_Speed[2] + motor_Speed[3]) * 0.354f / coefficient;
    cd->realvy        = -1 * (motor_Speed[0] - motor_Speed[1] - motor_Speed[2] + motor_Speed[3]) * 0.354f / coefficient;
    cd->realvw        = -1 * (motor_Speed[0] + motor_Speed[1] + motor_Speed[2] + motor_Speed[3]) * 0.25f / coefficient / CHASSIS_RADIUS;
}
// 此处为动力学逆解算
void Chassis_Calculate_Rotor_Torgue(ChassisData_Type *cd) {
    cd->rotorTorgue[0] = -1 * ((-cd->Fx + cd->Fy) * 0.354f + cd->T * 0.25f / CHASSIS_RADIUS) / CHASSIS_INVERSE_WHEEL_RADIUS;
    cd->rotorTorgue[1] = -1 * ((-cd->Fx - cd->Fy) * 0.354f + cd->T * 0.25f / CHASSIS_RADIUS) / CHASSIS_INVERSE_WHEEL_RADIUS;
    cd->rotorTorgue[2] = -1 * ((cd->Fx - cd->Fy) * 0.354f + cd->T * 0.25f / CHASSIS_RADIUS) / CHASSIS_INVERSE_WHEEL_RADIUS;
    cd->rotorTorgue[3] = -1 * ((cd->Fx + cd->Fy) * 0.354f + cd->T * 0.25f / CHASSIS_RADIUS) / CHASSIS_INVERSE_WHEEL_RADIUS;
}
...
float Chassis_Calculate_Power_Limit(float *motorCurrentOutput, int16_t *MCO_With_PowerLimit, float *realMotorSpeed, float targetPower) {
    ...
    for (int i = 0; i < 4; i++) {
        totalPower += (coefficient[0] + coefficient[1] * motorCurrentOutput[i] + coefficient[2] * realMotorSpeed[i] +
                       coefficient[3] * motorCurrentOutput[i] * realMotorSpeed[i] + coefficient[4] * motorCurrentOutput[i] * motorCurrentOutput[i] +
